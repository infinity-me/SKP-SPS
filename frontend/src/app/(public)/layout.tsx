  ...
  return (
    <footer className="bg-primary text-white py-20 px-6 border-t border-gold-500/10">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12">
        <div className="col-span-1 lg:col-span-2">
          <h2 className="text-2xl font-heading font-bold mb-2">
            SKP SAINIK <span className="text-gold-500">PUBLIC SCHOOL</span>
          </h2>
          <p className="text-white/40 text-xs font-bold uppercase tracking-widest mb-4">
            Affil. No. 2131950 | {s.affiliation} Board
          </p>
          ...
          <div className="space-y-1 text-sm text-white/50">
            <p>📞 {s.phone.split(",").map((p: string, i: number) => (
              <span key={i}>
                {i > 0 && " | "}
                <a href={`tel:${p.trim()}`} className="hover:text-gold-500 transition-colors">{p.trim()}</a>
              </span>
            ))}</p>
            <p>✉️ <a href={`mailto:${s.primaryEmail}`} className="hover:text-gold-500 transition-colors">{s.primaryEmail}</a></p>
        ...
        <div>
          ...
          <ul className="flex flex-col gap-2 text-white/70 text-sm">
            ...
            <li className="mt-4">
              <a href="/login?role=admin" className="text-xs font-bold uppercase tracking-widest text-white/40 hover:text-gold-500 transition-all border border-white/10 px-3 py-2 rounded-lg inline-block">
                Admin Login
              </a>
            </li>
          </ul>
        ...
        <div>
          ...
          <div className="flex flex-col gap-3">
            <a href="https://youtube.com/@sps_manihari?si=ZZ1-0mweYj9OgFcQ" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 group">
            ...
            <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(s.schoolName + " " + s.address)}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 group">
              <div className="w-9 h-9 rounded-xl bg-amber-500/20 flex items-center justify-center group-hover:bg-amber-500 transition-all">
                <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 text-amber-400 group-hover:text-white transition-colors"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>
              </div>
      ...
      <div className="max-w-7xl mx-auto mt-16 pt-8 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-4 text-white/30 text-xs">
        <p>© {new Date().getFullYear()} {s.schoolName}. All rights reserved.</p>
        <p>Dedicated to academic excellence and character building since {s.established}.</p>
      </div>
    </footer>
  )
}
